...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-surfacebook -g /boot/archiso-surfacebook.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_boot() {
    ...
    cp ${work_dir}/${arch}/airootfs/boot/archiso-surfacebook.img ${work_dir}/iso/${install_dir}/boot/${arch}/archiso-surfacebook.img
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-surfacebook ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz-surfacebook
}
...
make_efi() {
    ...
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-usb-surfacebook.conf > ${work_dir}/iso/loader/entries/archiso-x86_64-surfacebook.conf
...
make_efiboot() {
    ...
    cp ${work_dir}/iso/${install_dir}/boot/x86_64/vmlinuz-surfacebook ${work_dir}/efiboot/EFI/archiso/vmlinuz-surfacebook.efi
    cp ${work_dir}/iso/${install_dir}/boot/x86_64/archiso.img ${work_dir}/efiboot/EFI/archiso/archiso.img
    cp ${work_dir}/iso/${install_dir}/boot/x86_64/archiso-surfacebook.img ${work_dir}/efiboot/EFI/archiso/archiso-surfacebook.img
    ...
    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86_64-cd-surfacebook.conf > ${work_dir}/efiboot/loader/entries/archiso-x86_64-surfacebook.conf
